            ...
            <div className="px-4 py-6 sm:p-6 lg:p-8">
                <div className="grid grid-cols-2 gap-6 md:grid-cols-4 lg:grid-cols-1">
                    ...
                    <div>
                        ...
                        <p className="text-sm font-bold text-gray-900 mt-0.5">{orderDate}</p>
                    </div>
                   ...
                   return (
                    ...
                    <div className="flex flex-col justify-between flex-1 ml-5">
                        <div className="sm:grid sm:grid-cols-2 sm:gap-x-5">
                            ...
                            <p className="text-base font-bold text-left text-gray-900 sm:text-right">{item.quantity}</p>
                                <p className="text-base font-bold text-left text-gray-900 sm:text-right">{item.quantity*item.price}</p>
                            </div>
                        </div>

                        <div className="absolute bottom-0 left-0 sm:relative">
                            <div className="flex space-x-5">
                                <Link href={`/products/${slug}`} title="" className="p-1 -m-1 text-sm font-medium text-gray-500 transition-all duration-200 rounded hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900"> View Product </Link>

                                <span className="text-gray-200"> | </span>

                                <Link href={`/dashboard/orders/${order.id}/invoice`} title="" className="p-1 -m-1 text-sm font-medium text-gray-500 transition-all duration-200 rounded hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900"> Similar Product </Link>
                            </div>
                        </div>
                    </div>
                </li>
